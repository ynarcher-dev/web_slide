...
### 4.4 대시보드에 폴더를 끌어다 놓는 방법

Workers 생성 → **업로드 및 배포** 화면에 폴더를 끌어다 놓는 경로다.
**Cloudflare가 공식적으로 안내하는 방법이 아니며, 되면 좋고 안 되면 4.3으로 돌아간다.**

```bash
pnpm cf:dist   # dist/ 를 만든다
```
...
| 항목          | 설명                                                     |
| ------------- | -------------------------------------------------------- |
| `_worker.js`  | wrangler가 번들한 서버 코드 한 파일. 약 11MB, gzip 2.1MB |
| `_next/`      | 정적 자산                                                |
| `brand/`      | 로고                                                     |
| `favicon.ico` | 파비콘                                                   |

평소 배포에서는 wrangler가 이 번들링을 배포 시점에 대신 해 준다.
`.open-next/worker.js`는 4KB짜리 진입점일 뿐이고 실제 코드는 51MB 트리에 흩어져 있어서,
그대로 끌어다 놓으면 서버 코드가 실행되지 않는다. `pnpm cf:dist`는
`wrangler deploy --dry-run --outdir`로 그 번들링 결과만 미리 뽑아 낸다.

#### 업로드 후 반드시 할 것

**호환성 플래그에 `nodejs_compat`을 넣어야 한다.** 번들이 `node:` 내장 모듈을 149곳에서
쓰기 때문에, 플래그가 없으면 Worker가 시작하자마자 실패한다.
배포 설정 단계에 입력란이 없으면 업로드 후 설정 → 런타임에서 추가하고 다시 배포한다.
호환성 날짜는 `2026-08-25` 이상으로 둔다.

#### 동작 확인

`/login`이 열리고 로그인되는지를 본다. 여기가 서버 코드가 실제로 도는지 판별하는 지점이다.
첫 화면만 열리고 로그인에서 오류가 나면 서버 코드가 실행되지 않은 것이며,
대개 `nodejs_compat` 문제다.

`dist/`에 넣은 파일은 모두 공개 주소로 서빙되므로 안내 문서 같은 것을 함께 넣지 않는다.
