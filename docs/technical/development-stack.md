...
Storage는 이미지 슬라이드의 그림 파일 하나에만 쓴다.
...
슬라이드나 프레젠테이션을 삭제하면 쓰지 않게 된 이미지 파일도 함께 지운다.
그 밖의 외부 스토리지 서비스는 추가하지 않는다.
...
- `SlideRenderer`: 템플릿에 따라 표지 또는 본문 렌더러를 고르는 단일 진입점.
  `content`, `image`, `html`은 같은 본문 렌더러를 쓰고 콘텐츠 영역만 갈라진다
...
- `PresentationControls`: 화면 위쪽에 접혀 있는 발표 리모컨. 이전, 다음, 페이지 표시,
  콘텐츠 조작 모드, 전체화면, 발표 종료를 한 곳에 모은다
- `usePlayerNavigation`, `usePlayerKeyboard`, `useFullscreen`, `useControlsReveal`:
  이동, 키보드, 전체화면, 리모컨 표시 여부

리모컨은 `player.module.css`의 두 층으로 만든다. 바깥 층은 클릭을 받지 않고 안쪽 패널만 받는다.
접혀 있는 동안 슬라이드 클릭을 가로채면 콘텐츠 조작을 켤 수 없기 때문이다. 내려오는 조건은 세 가지다.
발표 시작 직후의 첫 노출, 포인터가 화면 위쪽 구역에 있을 때, 그리고 `:focus-within`이다.
앞의 둘은 `useControlsReveal`이, 마지막은 CSS가 처리한다. 웹페이지를 조작하는 동안에는 포인터 이벤트가
